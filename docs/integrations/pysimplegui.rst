...
By default the icon follows the button's own colors, so it grays out along with the label when the button is disabled. Pass ``reactive_states`` to say what each state does instead:
...
A mapping **replaces** the default behavior rather than adding to it — only the states it names react at all. Naming ``hover`` and ``pressed`` alone gives a button whose icon holds its resting color while the label beside it grays out, which is why ``disabled`` is named above.
