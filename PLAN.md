...
**Bytes take their color from PySimpleGUI, not from a literal.** `to_data()` cannot infer one — it has no widget, and `tkinter_icons` must not know PySimpleGUI exists — so the examples first hard-coded white, which is only right on a dark theme. They now read `sg.theme_text_color()` and `sg.theme_button_color_text()`, which need no window and are readable while the layout is still being written. The two are not interchangeable: under `DarkAmber`, text is `#fdcb52` and text on a button is `#000000`.

**The window icon is the one place the theme's foreground is wrong.** A title bar and taskbar are drawn by the OS, not on the theme's background, so `sg.theme_text_color()` there produced a white icon invisible against a light title bar. The capture uses a color picked to read on the desktop instead.
