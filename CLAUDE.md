...
**#112 is the only open issue, and it is now built and reviewed on `feat/psg-extension` — not merged.** The design is in the issue body, written 2026-08-12 against a real PySimpleGUI 6.3; the branch is `tkinter_icons.extensions.psg`, its tests, its docs page, and a captured screenshot. A first review round ran 2026-08-12 and its seven findings are fixed; `REVIEW.md` records them, pinned to the SHA they were read at. Read the issue rather than re-deriving it; what follows is only what a later session is most likely to get wrong.
