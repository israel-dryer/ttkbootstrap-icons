...
### Next action: decide whether #140 needs a round 4, scope `550f5ed..0bf7cf2`

**Round 3 ran and its findings are fixed. #140 is still unmerged and 5.1.0 is still uncut.** `REVIEW.md` carries all three rounds with every scope pinned to a SHA and every finding carrying a resolution; `PLAN.md` carries the intent, including the three changes that were never in the plan. **A session that arrives here does not merge and does not cut a tag** — those are the owner's calls, and #80 is why.

The open question is only whether `0bf7cf2` gets its own review round. It is the round-3 fix commit, written by the session that ran round 3, so by the protocol it is unreviewed. Arguing for stopping: rounds went 7 → 7 → 6 findings and round 3's were markedly smaller — one behavior defect, three instruments, one docs sentence, one declined — and it is the first fix diff that *deleted* public surface instead of adding to it. Arguing for a round 4: two of round 3's six findings were defects in round 2's own fixes, so this branch has a demonstrated habit of fix rounds introducing the next round's findings.

**The public behavior change in that diff is the removal of `on_missing`, which is the owner's call and not a review finding.** What a review is for is the second-order question, and it has now paid twice. The default flip turned up the `"none"` sentinel depending on the old default; the removal turned up the widget path reading the policy off `Icon` with the class named literally while `render_pil` read it off `cls`. Both were found by something failing rather than by anyone reasoning about it, which is the argument for asking a third time.
...
| #140 | **Written, reviewed three times, not merged.** Branch `fix/glyphmap-advertises-glyphs-the-font-lacks`, tip `0bf7cf2`. A glyph the font does not carry no longer renders blank in silence — and after round 3, neither does anything else. |
...
.venv-home/Scripts/python.exe -m pytest -q                       # 810 total, e.g. 795 passed 15 skipped
...
**5.1.0 — moved to the top of this section, since it is the live milestone.** #115, #136 and #91 are closed. #140 is written and reviewed three times but **not merged** — see the table at the top of this section. #112 has not been started.
