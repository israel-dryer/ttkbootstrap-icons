...
### Next action: #112, PySimpleGUI integration — the last open issue in the repository

**#140 merged 2026-08-10 as PR #141, after a fourth review round.** Everything 5.1.0 was waiting on is now on `main` except #112. **5.1.0 is deliberately uncut** — the owner's call the same day, made with the merge: the milestone is not ready to tag. Do not push `v5.1.0`.

**#112 is the only open issue, and its body is empty.** The title is *"Add PySimpleGUI support via extensions module"* and the milestone is 5.1.0; there is no written spec anywhere. The one piece of recorded intent is in "What #71 built" below — a PySimpleGUI integration is *one docs page*, using lazy factory functions because PSG does not guarantee a window up front, and it fits the existing model because `Icon.__init__` never touches Tk and rendering defers to first `.image` access. That is a sentence about docs structure, not a design.

**Settle one thing before designing anything: whether an integration is appropriate at all, on PySimpleGUI's current terms.** The issue predates any check of what that project is today, and its distribution and licensing have not been stable — so read the current PyPI page and license before writing code against a remembered version number, including which major version the "extensions module" in the title is meant to target. This is the same discipline the Meteocons call and the dead-docs-URL entry both came from: *check whether the thing exists as you remember it before writing down what it does.* If the answer makes an integration a bad idea, that is a finding for the issue, not a reason to build it anyway.

**Everything from the table down is finished work.** 5.0.0 and 5.0.1 both shipped, and so has all of 5.1.0 except #112; the sections that follow are kept because they record *how*, not because anything in them is outstanding.
...
| #112 | PySimpleGUI integration, scoped as 5.1 since #71. **Not started, empty issue body, and independent of everything else.** The next action, above. |

**Four rounds ran on #140, and the count is the least interesting thing about them.** 7 → 7 → 6 → 4 findings, and none of round 4's were in the source change: one docs example, two transcribed numbers, one missing changelog entry. Rounds 1–3 each turned up a live behavior defect and round 4 turned up none, which is what "done" looked like here — not a clean round, but a round whose findings had all moved out of the code and into the prose around it. `REVIEW.md` carries all four with every scope pinned to a SHA; it and `PLAN.md` are now on `main`, which is a **loose end worth a decision**: the next branch either starts them over or inherits four rounds of findings about work that has already shipped.
...
**5.1.0 — moved to the top of this section, since it is the live milestone.** #115, #136, #91 and #140 are all closed and merged. **#112 is the only one left, and the only open issue in the repository** — see the table at the top of this section. The milestone is complete apart from it, and uncut by decision.
