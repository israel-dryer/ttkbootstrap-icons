...
**Coverage is stored as sorted ranges, not as a set of codepoints.** 17.4 KB for all 31 styles against 5,792 KB, a factor of 333; membership is a `bisect` at 0.18 µs against a hash lookup at 0.04 µs. **State the definition with the number**: ranges are `Coverage.nbytes`, the bounds buffer alone, and sets are the old `frozenset` plus its int objects — measuring the array *object* instead gives 19.9 KB, which is how the first draft of this paragraph disagreed with itself. The extremes are also two different fonts: the biggest set is `fluent:filled` at 741 KB (two ranges, 16 bytes), the biggest bounds array is `fontawesome:solid` at 5,672 bytes (709 ranges, the most fragmented coverage shipped). `Coverage.nbytes` is public so the representation can be regression-tested, because every correctness test in the suite passes either way. The same change removed the unbounded format 12 enumeration that round 1 of the #140 review deferred as a nit: groups are emitted as ranges rather than walked.

**`on_missing` defaults to `"raise"` as of 5.1.0, not `"transparent"`.** The owner's call, 2026-08-09: a pack class already raised `ValueError` for a name it could not resolve, so the library answered one question two ways and the quiet answer wrote blank PNGs. `"warn"` and `"transparent"` remain for bulk renderers. **The `"none"` sentinel was silently depending on that default** — it drew a blank by falling through the missing-name path, so it had always raised for anyone who set `"raise"`. It is handled before the policy now, as `providers.NO_ICON`. When flipping a default, the question worth asking is not whether the new behavior is right but *what was quietly relying on the old one*.
