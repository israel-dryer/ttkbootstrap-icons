...
**#112 is the only open issue, and it now has a design in its body — written 2026-08-12, worked out against a real PySimpleGUI 6.3 and real windows.** It was empty until then. Read the issue rather than re-deriving it; what follows is only what a later session is most likely to get wrong.
...
**The prototype is `development/psg_icons.py`**, untracked and outside the package, with screenshots taken from real windows. It works on both the tk and ttk paths. The three findings most likely to be rediscovered the hard way are all on the issue, but the sharpest is this: **the attach must be triggered by an idle callback scheduled when the widget is created, not by `<Map>`.** PySimpleGUI packs the layout and then calls `master.update_idletasks()` to size and place the window, and `update_idletasks` **does not deliver events** — so a `<Map>` binding waits for the first real event-loop pass, by which time the window is already on screen, and the user watches the icons appear and the buttons resize. That is safe because PySimpleGUI never yields to the event loop while configuring an element, so the first idle after `element.Widget` is assigned is the first moment it is fully configured. There is no true per-element finalize hook to use instead: after the ttk style is applied, PySimpleGUI only *reads attributes* on the element, so a property getter would work today and break silently on a reordering.

**A `tk.Button` has no hover state, and reusing the word "active" across the two paths is a trap.** In ttk `active` means hover; on a `tk.Button` it means *pressed* — `tk::ButtonEnter` sets `-state active` only when the mouse button is already down. The prototype's public vocabulary is therefore `hover` / `pressed` / `disabled`, translated per path, which the owner chose 2026-08-12 over mirroring either toolkit's names.

**Still open on #112, and raised by the owner: theme awareness.** The prototype resolves every color once at attach time and never revisits it, so an application that re-themes after building keeps old icon colors on a re-themed button — visibly worse than never following the theme. `Icon.map` already tracks widgets and binds a theme listener, so the ttk path may be most of the way there for free; the tk path has no equivalent. The prior question, which is on the issue, is whether a PySimpleGUI re-theme reaches an existing window at all or only affects windows created afterward — establish that before building machinery for it.
