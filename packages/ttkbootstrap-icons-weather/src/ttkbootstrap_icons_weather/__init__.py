"""Superseded by ``tkinter-icons[weather]``.

This distribution had its final release as 1.0.1. Weather Icons now ship as an extra
of ``tkinter-icons``; see https://tkinter-icons.readthedocs.io/en/latest/getting-started/migrating.html
"""

import warnings

warnings.warn(
    "ttkbootstrap-icons-weather is superseded and will receive no further releases. "
    "Weather Icons now ship as an extra of tkinter-icons:\n"
    "\n"
    '    pip install "tkinter-icons[weather]"\n'
    "    from tkinter_icons import WeatherIcon\n"
    "\n"
    "The class name is unchanged; only the import root moves. "
    "See https://tkinter-icons.readthedocs.io/en/latest/getting-started/migrating.html",
    FutureWarning,
    stacklevel=2,
)

from .icon import WeatherIcon
from .provider import WeatherFontProvider

__all__ = ["WeatherFontProvider", "WeatherIcon"]
