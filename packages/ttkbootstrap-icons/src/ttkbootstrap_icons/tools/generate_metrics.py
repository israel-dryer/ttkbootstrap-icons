...
Run this whenever a provider's font or glyphmap changes::
    ...
    ttkicons-metrics --all              # every installed provider
    ttkicons-metrics fa mat             # named providers
    ttkicons-metrics bootstrap --check  # verify without writing
    ...
    for style in _provider_styles(provider):
        ...
        if check:
            ...
            if current == payload:
                ...
                print(f"  {state.upper():8s} {label}: run ttkicons-metrics {provider.name}")
                ok = False
            continue
