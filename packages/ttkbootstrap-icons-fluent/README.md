...
>
> Fluent System Icons now ship as an extra of **[tkinter-icons](https://pypi.org/project/tkinter-icons/)**:
>
> ```bash
...
> ```
>
> **[Fluent System Icons pack documentation](https://tkinter-icons.readthedocs.io/en/latest/packs/fluent.html)**
...
| | Before | Now |
|---|---|---|
| Install | `pip install ttkbootstrap-icons-fluent` | `pip install "tkinter-icons[fluent]"` |
| Import | `from ttkbootstrap_icons_fluent import FluentIcon` | `from tkinter_icons import FluentIcon` |

**The class is the same.** `FluentIcon` is still exported from `tkinter_icons`, so the only edit is the import line. Everything else — `(name, size, color, style)` — is unchanged.

The replacement pack tracks Fluent System Icons v1.1.261 and carries 12,879 names across the `regular`, `filled` and `light` styles.
...
**1.0.1 is the final release of `ttkbootstrap-icons-fluent`.** It ships the same font and the same glyph data as 1.0.0, and draws the same icons. Three things changed:
...
**Fluent System Icons** — [browse the set](https://github.com/microsoft/fluentui-system-icons) · [upstream license](https://github.com/microsoft/fluentui-system-icons/blob/main/LICENSE)
...
- [Fluent System Icons in the new documentation](https://tkinter-icons.readthedocs.io/en/latest/packs/fluent.html)
