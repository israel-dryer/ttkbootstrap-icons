"""
Icon Previewer for tkinter-icons
...
try:
    from tkinter_icons_bs import BootstrapIcon, BootstrapFontProvider
except ImportError:
    BootstrapIcon = None
...
class IconPreviewerApp:
    def __init__(self, root):
        self.root = root
        self.root.title("tkinter-icons browser")
    ...
    def _build_no_providers_ui(self):
        ...
        message = ttk.Label(
            main_frame,
            text="The tkinter-icons base package requires icon provider packages to function.\n\n"
        ...
        from tkinter_icons.packs import KNOWN_PACKS, PACKS_DOC_URL

        providers = [(pack.label, pack.install_command) for pack in KNOWN_PACKS[:5]]
        providers.append(("And more...", f"See {PACKS_DOC_URL}"))
...
def _set_app_icon(root):
    ...
    # `iconphoto` does not take a reference, and a collected PhotoImage leaves
    # the window with no icon — the same rule the docs give users about widgets.
